...
async def process_task_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    )  # Accessing the 'task' attribute of Task instances
    confirmation_message = (
        f"📝 You have successfully added the following task(s):\n\n{task_list_formatted}"
    )
...
def add_task_conversation_handler() -> ConversationHandler:
    return ConversationHandler(
        ...
        states={
            TASK_INFO: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_task_info)
            ]
        },
        fallbacks=[],
        map_to_parent={CHOOSE_PLAN_TASK: CHOOSE_PLAN_TASK},
    )
