...
async def schedule_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    unscheduled_tasks = [
        task for task in context.user_data.get("tasks", []) if not task.date_scheduled
    ]
    if unscheduled_tasks:
        tasks_list = "\n".join(
            [
                f"🔸 *{idx + 1}\.* {task.task}"
                for idx, task in enumerate(unscheduled_tasks)
            ]
        )
        instructions = """
📅 *Schedule Your Tasks*\n
...
{}
        ...
        await update.message.reply_text(
            "👍 All tasks are currently scheduled. 📅", reply_markup=plan_keyboard
        )
...
async def handle_task_selection(
    ...
    if user_input.isdigit() and 0 < int(user_input) <= len(unscheduled_tasks):
        ...
        if unscheduled_tasks:
            context.user_data[
                "task_queue"
            ] = unscheduled_tasks.copy()  # Create a queue of tasks to schedule
            return await schedule_next_task(update, context)
        else:
            await update.message.reply_text(
                "No unscheduled tasks to schedule.", reply_markup=plan_keyboard
            )
            return CHOOSE_PLAN_TASK
    else:
        await update.message.reply_text(
...
async def schedule_next_task(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    if context.user_data.get("task_queue"):
        ...
        await update.message.reply_text(
            "All tasks have been scheduled.", reply_markup=plan_keyboard
        )
        return CHOOSE_PLAN_TASK
...
async def show_schedule_options(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Scheduling task: {context.user_data['selected_task'].task}",
        reply_markup=schedule_keyboard,
    )


async def process_schedule_option(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> str:
    ...
    if selected_option == "Stop Scheduling":
        ...
        await update.message.reply_text(
            "⏎ Scheduling stopped. Returning to the main menu.",
        ...
        selected_task.add_date_to_history(
    ...
    if context.user_data.get("task_queue"):
        ...
        await update.message.reply_text(
            "⏩ Task scheduling skipped.", reply_markup=plan_keyboard
        )
        return CHOOSE_PLAN_TASK
    else:
        await update.message.reply_text(
            f"🙏 Thanks for scheduling your tasks! Now it's time to work! 🤓",
            reply_markup=plan_keyboard,
        )
        return CHOOSE_PLAN_TASK
